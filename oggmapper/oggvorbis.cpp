...
err page_header_read(FILE* fi, ogg_page_hdr* hdr)
{
	hdr->start_pos = ftell(fi);
	hdr->capture_pattern[0] = 0;
	if (fread_s(hdr->capture_pattern, 4, 4, 1, fi) != 1)
		return READ_ERROR;
	...
	if (fread_s(&hdr->stream_structure_version, 1, 1, 1, fi) != 1)
		return READ_ERROR;
	if (fread_s(&hdr->header_type_flag, 1, 1, 1, fi) != 1)
		return READ_ERROR;
	if (fread_s(&hdr->granule_pos, 8, 8, 1, fi) != 1)
		return READ_ERROR;
	if (fread_s(&hdr->serial, 4, 4, 1, fi) != 1)
		return READ_ERROR;
	if (fread_s(&hdr->seq_no, 4, 4, 1, fi) != 1)
		return READ_ERROR;
	if (fread_s(&hdr->checksum, 4, 4, 1, fi) != 1)
		return READ_ERROR;
	if (fread_s(&hdr->page_segments, 1, 1, 1, fi) != 1)
		return READ_ERROR;
	if (fread_s(hdr->segment_table, 256, 1, hdr->page_segments, fi) != hdr->page_segments)
		return READ_ERROR;
	return OK;
}
...
{
	err e;
	if (e = page_header_read(s->file, &s->cur_page), e != 0)
		return e;
	s->file_pos = ftell(s->file);
	s->next_segment = 0;
	return OK;
}
...
err vorbis_read_packet(vorbis_state* s)
{
	...
	do
	{
		...
		if (s->next_segment >= s->cur_page.page_segments)
		{
			...
			{
				fread(s->cur_packet.buf + packet_read, 1, packet_size - packet_read, s->file);
...
err vorbis_init(FILE* f, vorbis_state **out)
{
	err e;
	vorbis_state *s = (vorbis_state*)malloc(sizeof(vorbis_state));
	if (!s) return MALLOC;
	s->file = f;
	s->next_sample = 0;
	s->file_pos = 0;
